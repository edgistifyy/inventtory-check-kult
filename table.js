function table(rootEl) {
  ...
  this.tr
    ...
    .forEach((each) => this.editableKeys.push(each.getAttribute("data")));
  this.mode = 0;
  this.page = 0;
  this.perpage = 10;
  ...
  this.getJson = function (b) {
    return this.searching ? this.oldJson : this.json;
  };
  ...
  this.updateIds = function () {
    this.getJson().forEach((each, i) => (each.temp_id = i + 1));
  };
  this.newEntry = function () {
    var nE = this.getJson()[0],
      promise;
  ...
  this.fillData = function (obj) {
    ...
    tr.querySelectorAll("[data]").forEach((dataEl) => {
      ...
      if (dataEl.tagName == "IMG") {
        dataEl.setAttribute("src", obj[key]);
      } else if (dataEl.tagName == "INPUT") {
        dataEl.setAttribute("value", obj[key]);
      } else {
        dataEl.innerHTML = obj[key] || "";
      }
  ...
  this.totalPage = function () {
    return Math.ceil(this.json.length / this.perpage);
  };
  ...
  this.changePage = function (d) {
    var el = this.root.querySelector(".pageField"),
  ...
  this.searchArray = function searchArray(json, key) {
    var searched = json.filter((each) => {
      return (
        Object.values(each).join("").toLowerCase().indexOf(key.toLowerCase()) >=
        0
      );
    });
    return searched;
  ...
  this.interactivity = function () {
    //searching system
    var search = this.root.querySelector(".searchField"),
      stimeout,
      obj = this;
    if (search) {
    ...
    pagiField.onchange = function (e) {
      if (e.target.value != "") {
        obj.changePage(parseInt(e.target.value));
      }
    };
    obj.pagiTextRender();
    ...
    root.onclick = (e) => {
      ...
      if (add != null) {
        obj.newEntry().then(function () {
          ...
          obj.loadALL();
          if (obj.mode == 0) {
            obj.root.querySelector(".edit").click();
          }
        });
      }
      if (edit != null) {
